...
end entity fetch_unit_32bit;

architecture structural of fetch_unit_32bit is
    ...
    component sign_extend_32bit is
        port (
            i_imm16 : in  std_logic_vector(15 downto 0);
...
begin
    ...
    sign_ext: sign_extend_32bit
        port map (
            i_imm16 => i_imm16,
            o_imm32 => imm32
        );

    o_sign_ext_imm <= imm32;

    sll_branch: shift_left_2
        port map (
            i_input  => imm32(7 downto 0),
    ...
    beq_taken <= i_branch and i_zero;
    bne_taken <= i_bne and (not i_zero);
    pc_src    <= beq_taken or bne_taken;
