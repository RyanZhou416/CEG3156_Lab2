...
architecture structural of alu_32bit is
    ...
    signal set_out      : std_logic_vector(31 downto 0);
    signal overflow_int : std_logic;
    signal slt_result   : std_logic;
    signal or_all       : std_logic_vector(31 downto 0);

begin

    carry_chain(0) <= i_control(2);

    overflow_int <= carry_chain(31) xor carry_chain(32);
    slt_result   <= set_out(31) xor overflow_int;
    ...
    or_all(0) <= result_int(0);
    gen_or: for i in 1 to 31 generate
        or_all(i) <= or_all(i - 1) or result_int(i);
    end generate gen_or;
    o_zero <= not or_all(31);
